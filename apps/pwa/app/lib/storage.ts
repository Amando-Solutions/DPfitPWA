...
// --- Firestore `Timestamp` round-tripping ------------------------------------
//
// Every instant in the app is a `Timestamp`, which JSON has no idea about. Two
// details make this less obvious than it looks:
//
//   - `Timestamp` has its own `toJSON()`, and `JSON.stringify` calls it *before*
//     the replacer runs. By the time a plain replacer sees the value it is
//     already an untagged `{ seconds, nanoseconds }`, indistinguishable from an
//     object that happens to have those two keys. Reading the original back off
//     the holder (`this[key]`) is what gets ahead of that.
//   - Seconds and nanoseconds are kept rather than milliseconds, so a value
//     written here and read back is the same instant to the nanosecond.

const TS_TAG = '$firestoreTimestamp'

type TaggedTimestamp = { [TS_TAG]: [seconds: number, nanoseconds: number] }

function tagTimestamps(this: Record<string, unknown>, key: string, value: unknown) {
  // `this` is the object (or array) the value came from, holding it untouched.
  const original = this[key]
  return original instanceof Timestamp
    ? ({ [TS_TAG]: [original.seconds, original.nanoseconds] } as TaggedTimestamp)
    : value
}

const reviveTimestamps = (_key: string, value: unknown) => {
  if (typeof value !== 'object' || value === null || !(TS_TAG in value)) return value
  const [seconds, nanoseconds] = (value as TaggedTimestamp)[TS_TAG]
...
export const storage = {
  read<T>(key: string, fallback: T): T {
    ...
    try {
      return JSON.parse(raw, reviveTimestamps) as T
    } catch {
      return fallback
    }
  },
  ...
  write<T>(key: string, value: T): boolean {
    ensureVersion()
    const full = fullKey(key)
    const raw = JSON.stringify(value, tagTimestamps)
    let persisted = false
