...
export const challengeClock = (
  ...
  // `weekThemes` is a non-empty tuple, so index 0 is a real theme, not a maybe.
...
export const weekOf = (joinedAt: Timestamp, at: Timestamp): number =>
  Math.min(Math.floor(daysSince(joinedAt, at.toDate()) / 7) + 1, challenge.totalWeeks)
